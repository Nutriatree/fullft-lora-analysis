...
from pubmedqa.reporting.style import apply_report_style, save_figure


CHECKPOINTS = (25, 50, 75, 100)
RANK_RUNS = (("L3", "lora-r4", 4, "#9467BD"), ("L1", "lora", 8, "#1F77B4"), ("L4", "lora-r16", 16, "#FF7F0E"))
...
def final_update_file(run_dir: Path) -> Path:
    return run_dir / "layerwise_updates" / "scheduled_pct_100_epoch_001_step_001250.jsonl"


def checkpoint_update_file(run_dir: Path, percent: int) -> Path:
    steps = {25: 313, 50: 625, 75: 938, 100: 1250}
    return run_dir / "layerwise_updates" / f"scheduled_pct_{percent:03d}_epoch_001_step_{steps[percent]:06d}.jsonl"


def save(figure: plt.Figure, output_dir: Path, name: str, dpi: int) -> tuple[Path, Path]:
...
def draw_a1_selection(lora_dir: Path, output_dir: Path, dpi: int) -> None:
    ...
    colors = ["#2CA02C" if layer in high["target_layers"] else "#F58518" if layer in low["target_layers"] else "#BDBDBD" for layer in layers]
    ...
    axis.legend(
        handles=[
            plt.Rectangle((0, 0), 1, 1, color="#2CA02C", label="LL1 high-update layers"),
...
def matrix(records: list[dict], modules: list[str]) -> np.ndarray:
    ...
    for record in records:
        ...
        if module in positions:
            result[record["layer_index"], positions[module]] = record["relative_update_norm"]
    return result


def draw_a2_final_heatmaps(full_dir: Path, lora_dir: Path, output_dir: Path, dpi: int) -> None:
...
def draw_a3_temporal_heatmaps(lora_dir: Path, output_dir: Path, dpi: int) -> None:
    ...
    for axis, module in zip(axes, ("Q", "V")):
        ...
        axis.set_yticks(range(len(CHECKPOINTS)), [f"{percent}%" for percent in CHECKPOINTS])
...
def draw_a4_update_dynamics(lora_dir: Path, output_dir: Path, dpi: int) -> None:
    shares = read_json(lora_dir / "analysis" / "module_update_share.json")["checkpoint_timeline"]
    performance = read_json(lora_dir / "analysis" / "performance_dynamics.json")["checkpoint_timeline"]
    progress = [entry["checkpoint_percent"] for entry in shares]
    q_cumulative = [entry["by_module"]["Q"]["sum_delta_w_relative_norm"] for entry in shares]
    v_cumulative = [entry["by_module"]["V"]["sum_delta_w_relative_norm"] for entry in shares]
    q_share = [entry["by_module"]["Q"]["sum_cumulative_update_share"] for entry in shares]
    v_share = [entry["by_module"]["V"]["sum_cumulative_update_share"] for entry in shares]
    ...
    f1_axis.plot(progress, f1, marker="s", linestyle="--", color="#333333", label="Validation Macro-F1")
    ...
    figure.suptitle("LoRA Update Growth, Module Allocation, and Validation Performance", y=1.02)
...
def draw_a5_effective_rank(study_dir: Path, output_dir: Path, dpi: int) -> None:
    ...
    for tag, directory, rank, color in RANK_RUNS:
        timeline = read_json(study_dir / "Qwen_Qwen3-1.7B" / directory / "analysis" / "lora_effective_rank.json")["checkpoint_timeline"]
        progress = [entry["checkpoint_percent"] for entry in timeline]
        means = [np.mean([record["effective_rank"] for record in entry["records"]]) for entry in timeline]
        stds = [np.std([record["effective_rank"] for record in entry["records"]]) for entry in timeline]
        axis.plot(progress, means, marker="o", color=color, label=f"{tag}: configured r={rank}")
        axis.fill_between(progress, np.asarray(means) - np.asarray(stds), np.asarray(means) + np.asarray(stds), color=color, alpha=0.13)
...
def draw_a6_singular_spectrum(study_dir: Path, output_dir: Path, dpi: int) -> None:
    figure, axis = plt.subplots(figsize=(8.2, 4.7), constrained_layout=True)
    for tag, directory, rank, color in RANK_RUNS:
        timeline = read_json(study_dir / "Qwen_Qwen3-1.7B" / directory / "analysis" / "lora_singular_values.json")["checkpoint_timeline"]
        records = timeline[-1]["records"]
        singular_values = np.asarray([record["singular_values"][:rank] for record in records], dtype=float)
        mean_values = singular_values.mean(axis=0)
        normalized = mean_values / mean_values.sum() if mean_values.sum() > 0 else mean_values
        axis.plot(range(1, rank + 1), normalized, marker="o", color=color, label=f"{tag}: r={rank}")
