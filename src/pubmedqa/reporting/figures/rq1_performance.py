...
from pubmedqa.domain.metrics import classwise_f1
from pubmedqa.reporting.readers import RunArtifacts, StudyArtifactReader
from pubmedqa.reporting.style import annotate_bars, apply_report_style, save_figure
...
def generate_rq1_performance_figure(
    ...
    *,
    ...
    figure, (overall_axis, class_axis) = plt.subplots(1, 2, figsize=(12.5, 4.5), constrained_layout=True)
    ...
    for index, (tag, label, color) in enumerate(RUNS):
        values = [float(metrics[tag]["class_f1"][class_label]) for class_label in LABELS]
        bars = class_axis.bar(
