...
def generate_rq2_tradeoff_figure(
    ...
    *,
    ...
    values = {tag: load_run(study_dir, directory) for tag, (directory, _, _) in RUNS.items()}
    ...
    axis.plot(rank_x, rank_y, color="#1F77B4", linewidth=1.8, zorder=2, label="Q,V rank sweep")
    for tag in rank_tags:
        ...
        size = 105 + 22 * rank
        axis.scatter(x, y, s=size, color="#1F77B4", edgecolor="white", linewidth=1.2, zorder=3)
        axis.annotate(
