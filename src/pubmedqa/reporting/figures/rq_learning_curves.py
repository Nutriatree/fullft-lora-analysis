...
from pubmedqa.reporting.readers import RunArtifacts, StudyArtifactReader
from pubmedqa.reporting.style import apply_report_style, moving_average, save_figure
...
RQ_RUNS = {
    ...
    "rq2": ("RQ2: LoRA target-module and rank configurations", ("lora", "lora-qkvo", "lora-r4", "lora-r16")),
    "rq3": ("RQ3: All-layer and selective-layer LoRA", ("lora", "selective-lora-high-update", "selective-lora-low-update")),
}
...
def draw_rq_figure(
    *,
    ...
    for run in runs:
        ...
        train_losses = np.asarray([entry["train_loss"] for entry in train_timeline], dtype=float)
        train_axis.plot(steps, train_losses, color=train_color, alpha=0.10, linewidth=0.5)
        train_axis.plot(
            steps,
        ...
        validation_losses = np.asarray([entry["validation_loss"] for entry in checkpoints], dtype=float)
    ...
    for step in CHECKPOINT_STEPS:
        train_axis.axvline(step, color="#bdbdbd", linestyle="--", linewidth=0.7, zorder=0)
...
def generate_rq_learning_curves(
    ...
    *,
    ...
    for rq_key in RQ_RUNS:
        generated.extend(draw_rq_figure(
            model_dir=model_dir,
